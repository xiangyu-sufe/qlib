...
from ..loss.loss import (ic_loss, rankic_loss,
                         topk_ic_loss, topk_rankic_loss,
                           ranking_loss, pairwise_loss, mse,
                            topk_return)
from ..loss.ndcg import calculate_ndcg_optimized, compute_lambda_gradients
    ...
    def train_epoch(self, data_loader):
        ...
        for i, (data, news, news_mask) in enumerate(data_loader):
            ...
            if isinstance(loss_dict, dict):
                ...
                if self.debug:
                    print(loss_dict['total_loss'].item(),
                          loss_dict['router_loss'].item(),
                          loss_dict['expert_loss'].item())
            else:
                loss = loss_dict
            ...
            lambda_grads = 0.8 * lambda_grads + grad * 0.2
            pred.backward(lambda_grads)                
            torch.nn.utils.clip_grad_norm_(self.MIGA_model.parameters(), 3.0) 
