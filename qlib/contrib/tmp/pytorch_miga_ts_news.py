    ...
    def train_epoch(self, data_loader):
        ...
        for i, (data, news, news_mask) in enumerate(pbar):
            ...
            if isinstance(loss_dict, dict):
                ...
                loss = loss_dict
            # print(loss_dict['total_loss'].item(), loss_dict['router_loss'].item(), loss_dict['expert_loss'].item())
            ...
            for name in self.display_list:
                result['train_'+name].append(self.metric_fn(pred.detach(), label.detach(), hidden = routing_weights, name = name))
